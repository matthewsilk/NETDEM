...
#Generate population
pop_info<-population_generation_basic(n=200,ng=200,plot=FALSE)
...
for(ds in 1:dem_steps){
  ...
  if(ds==1){
    pre_cap<-seq(1,length(indiv_data$indivs),1)
  }
...
survival<-nimbleModel(code = nim_mod2,
                      constants=my.constants,
                      data = my.data,
                      inits = initial.values(),
                      calculate = FALSE)
...
FULL_RES_C<-mcmc.output

caught<-apply(y,2,sum)[2:11]
pres<-unlist(lapply(ts_surv2,nrow))
pop_sum<-data.frame(caught,pres)

results<-list(SAMP_RES_C,SAMP_RES_L,FULL_RES_C,FULL_RES_L,ts_surv2,pop_sum)
