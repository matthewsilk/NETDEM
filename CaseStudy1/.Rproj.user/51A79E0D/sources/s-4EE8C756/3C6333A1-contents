##Script 1. Betweenness centeality with various levels of sampling and various
##effect sizes
...
#75% of groups detected
#100% of individuals in detected groups observed
#Simulated eff = -0.75 on betweenness

reps<-20

SAMP_RESN3<-FULL_RESN3<-array(NA,dim=c(4,7,reps))
...
for(r in 12:reps){
  ...
  for(ds in 1:dem_steps){
    ...
    indiv_data<-covariates_survival(indiv_data=indiv_data,indiv_info=indiv_info,network=network,
                                    ...
                                    net_vars="betweenness",net_effs=list(-0.75),net_packages="igraph",scale_net=TRUE,
                                    net_cov=FALSE,
                                    mps=0.8,lvps=0.5)
  ...
  #model1 <- nimbleModel(code = hmm.survival4,
  ...
  SAMP_RESN3[,,r]<-as.matrix(store_res)
  ...
  mcmc.output2 <- nimbleMCMC(code = hmm.survival6,
                             constants = my.constants,
                             data = my.data,
                             inits = initial.values,
                             monitors = parameters.to.save,
  ...
  FULL_RESN3[,,r]<-as.matrix(store_res2)
  
  print(r)
  
}

#saveRDS(SAMP_RESN3,"SAMP_RESN3.RDS")
#saveRDS(FULL_RESN3,"FULL_RESN3.RDS")

par(xpd=FALSE)
plot(SAMP_RESN3[3,1,]~FULL_RESN3[3,1,],xlim=c(-1.5,0.25),ylim=c(-1.5,0.25))
lines(x=seq(-100,100,1),y=seq(-100,100,1),lty=3)
lines(x=rep(-0.75,2),y=c(-100,100),lty=1)
lines(y=rep(-0.75,2),x=c(-100,100),lty=1)
cor.test(FULL_RESN3[3,1,],SAMP_RESN3[3,1,])

mean(FULL_RESN3[3,1,],na.rm=T)
median(FULL_RESN3[3,1,],na.rm=T)

mean(SAMP_RESN3[3,1,],na.rm=T)
median(SAMP_RESN3[3,1,],na.rm=T)
