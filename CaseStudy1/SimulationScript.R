...
SAMP_RESN3<-FULL_RESN3<-array(NA,dim=c(4,7,nrow(pars1)))
...
for(i in 1:nrow(pars1)){
  ...
  for(ds in 1:dem_steps){
    ...
    ifelse(net_vars=="strength",clo_tmp<-eval(parse(text=paste0("igraph::",net_vars[i],"(network2)"))),eval(parse(text=paste0("igraph::",net_vars[i],"(network2,weights=1/E(network2)$weight,normalized=TRUE)"))))
    ...
    ifelse(net_vars=="strength",clo_tmp<-eval(parse(text=paste0("igraph::",net_vars[i],"(network_obs2)"))),eval(parse(text=paste0("igraph::",net_vars[i],"(network_obs2,weights=1/E(network_obs2)$weight,normalized=TRUE)"))))
