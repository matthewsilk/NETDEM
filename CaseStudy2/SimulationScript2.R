...
SAMP_RES<-matrix(NA,nr=500,nc=4)
...
#This number need to be flexible now
clo<-matrix(NA,nr=1000,nc=dem_steps)
clo_tr<-matrix(NA,nr=1000,nc=dem_steps)

start_obs<-2
end_obs<-max(seq(start_obs,20,by=seq_lengths[pcgs==pcg]))
interval_obs<-seq_lengths[pcgs==pcg]

trait_data<-indiv_info[[1]]
...
for(ds in 1:dem_steps){
    ...
    if(ds>1){
      if(ds==2){
        ...
        names(pre_cap_t)<-as.character(prev_data$indivs)
      }
      if(ds>2){
    ...
    ts_surv2[[ds]]<-cbind(indiv_data$indivs,indiv_data$survival)
    ...
    trait_data<-rbind(trait_data,indiv_info[[1]][indiv_info[[1]]$ID%in%trait_data$ID==FALSE,])
...
y<-capture_data[[1]]
clo<-clo[1:nrow(full_indiv_data),]

never<-which(rowSums(y[,2:11])==0)
y<-y[-never,]
clo<-clo[-never,]
trait_data<-trait_data[-never,]

get.first<-function(x) min(which(x!=0))
f<-apply(y[,2:11],1,get.first)
...
my.constants <- list(N = nrow(y), T = ncol(y)-1, f=f, sex=as.numeric(trait_data$sex),
...
ch.init<-function(z,f){
  for(j in 1:nrow(z)){z[j,1:f[j]]<-NA}
  return(z)
}

zinits2<-ch.init(zinits,f)
...
survivalConf <- configureMCMC(survival,monitors = parameters.to.save,print=FALSE)
survivalMCMC <- buildMCMC(survivalConf,useConjugacy = FALSE)
CsurvivalMCMC <- compileNimble(survivalMCMC,
                               project = survival)

mcmc.output <- runMCMC(mcmc = CsurvivalMCMC,niter=n.iter,nburnin=n.burnin,nchains=n.chains,thin=thin,progressBar=FALSE)
...
SAMP_RES<-mcmc.output

caught<-apply(y,2,sum)[2:11]
pres<-unlist(lapply(ts_surv2,nrow))
pop_sum<-data.frame(caught,pres)

output<-list(SAMP_RES,ts_surv2,pop_sum)

eval(parse(text=paste0("saveRDS(output,'results/res",i,".RDS')")))
